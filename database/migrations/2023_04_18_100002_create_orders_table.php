...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('orders', function (Blueprint $table) {
            ...
            $table->date('pend_arrive_day')->comment('仮到着希望日')->nullable();
            $table->date('use_end_day')->comment('使用最終日')->nullable();
            $table->date('order_use_from')->comment('占有開始日')->nullable();
            $table->date('order_use_to')->comment('占有最終日')->nullable();
            $table->integer('token')->comment('重複対策トークン');
            $table->boolean('event_venue_pending')->nullable()->comment('送付先住所後送フラグ');
